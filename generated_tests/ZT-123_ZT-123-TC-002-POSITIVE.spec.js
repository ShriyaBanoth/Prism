...
test.describe('ZT-123: User logs in with valid credentials and special characters', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(baseUrl);
    // Precondition: User is on the login page
    // Precondition: User has a valid registered account with special characters
    // Precondition: Browser cookies are enabled
  });

  test('User logs in with valid credentials and special characters', async ({ page }) => {
    // Step 1: Navigate to the login page
    // Expected: Login page loads successfully with email and password fields visible
    await page.goto(baseUrl);
    // Step 2: Enter valid email address with special characters in the email field
    // Expected: Email field is populated with the entered email
    await page.fill('#username', 'test.user@example.com');
